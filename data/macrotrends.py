...
#
# https://www.macrotrends.net/assets/php/stock_data_download.php?s=655ae7d19388f&t=ODD
#
...
def get_financial_statement(
    tkr: str, company_name: str, statement_type: str
) -> pd.DataFrame:
    res = requests.get(
        f"https://www.macrotrends.net/stocks/charts/{tkr}/{str}/{statement_type}",
        headers={"User-Agent": "b2g"},
    )
    ...
    data_line = list(
        filter(lambda l: "originalData" in l and "=" in l, data_script.split("\n"))
    )[0]
...
def init_model(
    tkr: str, company_name: str, path: str = ".", to_excel: bool = False
) -> None:
    """
    ...
    df = (
        income_statement.join([balance_sheet, cash_flow]).transpose().sort_index(axis=1)
    )
...
def run(args=None):
    ...
    parser.add_argument(
        "--company_name", required=True, help="Company name from macrotrends URL"
    )
